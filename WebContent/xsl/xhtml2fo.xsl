...
<xsl:stylesheet version="1.0"
	...
	<xsl:template name="common-atts">
		<xsl:copy-of select="@id|@color|@xml:lang" />
		<xsl:if test="@align">
		...
		<fo:root>
			...
			<fo:page-sequence master-reference="page">
				<fo:static-content flow-name="xsl-region-before">
					<fo:block display-align="after" padding-before=".2in"
	...
	<xsl:template
		match="xhtml:head|head|xhtml:applet|applet|xhtml:area|area|xhtml:base|base
	...
	<xsl:template match="xhtml:a[@href]|a[@href]">
		<fo:block>
			<fo:basic-link color="blue" text-decoration="underline">
				<xsl:if test="@type">
					<xsl:attribute name="content-type"><xsl:value-of
						select="@type" /></xsl:attribute>
				</xsl:if>
				<xsl:choose>
					<xsl:when test="starts-with(@href,'#')">
						<xsl:attribute name="internal-destination"><xsl:value-of
							select="substring-after(@href,'#')" /></xsl:attribute>
					</xsl:when>
					<xsl:otherwise>
...
<xsl:template match="xhtml:img|img|xhtml:input[@type='image']|input[@type='image']">
	...
	<fo:block space-after="10pt">
		  ...
		  <xsl:if test="@height"><xsl:attribute name="height"><xsl:value-of select="concat(string(@height div 72),'cm')"/></xsl:attribute></xsl:if>
		  <xsl:if test="@width"><xsl:attribute name="width"><xsl:value-of select="concat(string(@width div 72),'cm')"/></xsl:attribute></xsl:if>
	   	  <xsl:call-template name="common-atts"/>
  		</fo:external-graphic>
  	</fo:block>
</xsl:template>
	  ...
	  <fo:table-body>
	    <fo:table-row>
	      ...
	      <fo:table-cell>
			  ...
			  <fo:table border-width="0.2mm" border-style="solid"><xsl:call-template name="common-atts"/>
			  	<xsl:attribute name="display-align"><xsl:value-of select="@align"/></xsl:attribute>	
				<xsl:if test="@width"><xsl:attribute name="width"><xsl:value-of select="concat(string(@width div 72),'cm')"/></xsl:attribute></xsl:if>  
...
<xsl:template name="mock-col">
  ...
  <xsl:if test="$cols&gt;0">
    <fo:table-column column-width="proportional-column-width(1)" border-right-width="0.2mm" border-right-style="solid"/>
    <xsl:call-template name="mock-col">
...
<xsl:template match="xhtml:tr|tr">
  <fo:table-row border-top-width="0.2mm" border-top-style="solid" ><xsl:call-template name="common-atts"/>   
	<xsl:attribute name="height"><xsl:value-of select="concat(string(@height div 72),'cm')"/></xsl:attribute>   
    <xsl:apply-templates/>
  </fo:table-row>
</xsl:template>
...
<xsl:template match="xhtml:ul/xhtml:li|ul/li|xhtml:menu/xhtml:li|menu/li">
  <fo:list-item><xsl:call-template name="common-atts"/>
    <fo:list-item-label end-indent="label-end()">
      <fo:block text-align="end">
        <fo:block font-family="{$font.symbol}">
          <xsl:choose>
...
<xsl:template match="xhtml:hr|hr">
  <fo:block>  
  <xsl:if test="@width"><xsl:attribute name="leader-length"><xsl:value-of select="@width"/></xsl:attribute></xsl:if>
  <fo:leader leader-pattern="rule" rule-style="groove" content-width="scale-to-fit">
    <xsl:if test="@size">
...
<xsl:template match="xhtml:big|big">
  <fo:block font-size="larger"><xsl:call-template name="common-atts"/>
    <xsl:apply-templates/>
  </fo:block>
</xsl:template>

<xsl:template match="xhtml:blink|blink|xhtml:marquee|marquee">
  <fo:block background-color="yellow"><xsl:call-template name="common-atts"/>
    <xsl:apply-templates/>
  </fo:block>
</xsl:template>
...
<xsl:template match="xhtml:code|code|xhtml:kbd|kbd|xhtml:tt|tt">
  <fo:block font-family="monospace"><xsl:call-template name="common-atts"/>
    <xsl:apply-templates/>
  </fo:block>
</xsl:template>

<xsl:template match="xhtml:del|del|xhtml:s|s|xhtml:strike|strike">
  <fo:block text-decoration="line-through"><xsl:call-template name="common-atts"/>
    <xsl:apply-templates/>
  </fo:block>
</xsl:template>
...
<xsl:template match="xhtml:ins|ins|xhtml:u|u">
  <fo:block text-decoration="underline"><xsl:call-template name="common-atts"/>
    <xsl:apply-templates/>
  </fo:block>
</xsl:template>

<xsl:template match="xhtml:nowrap|nowrap">
  <fo:block wrap-option="no-wrap"><xsl:call-template name="common-atts"/>
    <xsl:apply-templates/>
  </fo:block>
</xsl:template>
...
<xsl:template match="xhtml:small|small">
  <fo:block font-size="smaller"><xsl:call-template name="common-atts"/>
    <xsl:apply-templates/>
  </fo:block>
</xsl:template>
...
<xsl:template match="xhtml:sub|sub">
  <fo:block baseline-shift="sub"><xsl:call-template name="common-atts"/>
    <xsl:apply-templates/>
  </fo:block>
</xsl:template>

<xsl:template match="xhtml:sup|sup">
  <fo:block baseline-shift="super"><xsl:call-template name="common-atts"/>
    <xsl:apply-templates/>
  </fo:block>
</xsl:template>
...
<xsl:template match="xhtml:input[@type='checkbox']|input[@type='checkbox']">
  <fo:block font-family="{$font.symbol}" font-size="larger"><xsl:call-template name="common-atts"/>
    <xsl:choose>
