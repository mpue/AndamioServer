 ...
 * 
 */
public class AsyncUploadPlugin extends AbstractPlugin implements IPlugin, Serializable {

	private static final long serialVersionUID = -5835496142515212802L;
	protected static Log log = LogFactory.getLog(AsyncUploadPlugin.class);

	private Config config;

	public AsyncUploadPlugin() {
		super();
		pluginID = "AsyncUploadPlugin";
		...
		paramDescriptors.put("plugin_upload_domain", resourceBundle.getString("plugin.upload.domain"));
	}
	...
	public String getContent() {
		...
		String domain = paramMap.get("plugin_upload_domain");
		
		if (directory == null || "".equals(directory)) {
			return "No upload directory specified.";		
		}
		else {
			request.getSession().setAttribute("uploadDirectory", directory);		
		}

		request.getSession().setAttribute("uploadDomain", domain);
