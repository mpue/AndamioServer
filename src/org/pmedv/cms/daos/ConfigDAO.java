...
public class ConfigDAO extends AbstractDAO{
	...
	protected String getQueryAll() {
		return "from Config config order by config.id";
	}

	@Override
	protected String getQueryById() {
		return "from Config config where config.id = ?";
	}

	@Override
	protected String getQueryByName() {
		return "from Config config where config.sitename like ? order by config.sitename";
	}
